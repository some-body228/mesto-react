    ...
    return (
        <li className="elements__card">
            ...
            <div className="elements__like-container">
                <button className={`elements__like-button ${isLiked && "elements__like-button_liked"}`} type="button" onClick={handleLikeClick}></button>
                <span className="elements__like-number">{props.curCard.likes.length}</span>
            </div>
            <button className="elements__trash-button" type="button" style={{display:isOwn? "block": "none" }} onClick={handleDeleteClick}></button>
        </li>
    )
}
export default Card